...
# To run my code for x sec and delay text
import time
...
# For changing the colo r of text
def change_color(user_word, word):
    for i in range(len(user_word)):
        if user_word[i] == word[i]:
            print(Fore.GREEN + user_word[i], end="")
            print('\033[39m', end="")  # no newline
            time.sleep(0.35)
        elif user_word[i] != word[i] and user_word[i] in word:
            print(Fore.YELLOW + user_word[i], end="")
            print('\033[39m', end="")  # no newline
            time.sleep(0.35)
        else:
            print(Fore.RED + user_word[i], end="")
            print('\033[39m', end="")  # no newline
            time.sleep(0.35)
...
def mode(length_of_word):
    ...
    while user_word.lower() != word:
        ...
        if(len(user_word) != length_of_word):
            ...
            change_color(user_word, word)
        display_keyboard(user_word, word, guessed_letters)  
        if(user_word != word):
            score -= 2
            hint(count,word)
...
def two_player(p1_name, p2_name):
    ...
    while (p1.lower() != word or p2.lower() != word):
        ...
        if(len(p1) != 6):
            ...
            change_color(p1 , word)
        display_keyboard(p1, word, guessed_letters)
        ...
        if(len(p2) != 6):
            ...
            change_color(p2, word)
        display_keyboard(p2, word, guessed_letters)
        if(p1 != word):
            hint(p2_count,word)
        print("\t"*len(p2_name))
        print()
...
def display_keyboard(user_word, word, guessed_letters):
    ...
    for i in range(len(user_word)):
        letter = user_word[i].upper()
        
        if user_word[i] == word[i]:
            guessed_letters[letter] = "CORRECT"
        elif user_word[i] in word:
            guessed_letters[letter] = "PRESENT"
        else:
            guessed_letters[letter] = "ABSENT"

    
    print("\n--- AVAILABLE LETTERS ---")
    for row in rows:
        # Indentation spacing to simulate a real keyboard layout
        if row.startswith("A"):
            print(" ", end="")
        if row.startswith("Z"):
            print("   ", end="")
        ...
        for letter in row:
            if letter in guessed_letters:
                ...
                if status == "CORRECT":
                    ...
                    print(Style.DIM + letter + Style.RESET_ALL, end=" ")
            else:
